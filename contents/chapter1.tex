...
最新适配物理与天文学院的格式要求，调整了参考文献的引用格式并添加文献类型标识，将中文与西文之间的一个半角字符的自动间距关闭。\texttt{$\backslash$texttt}命令只在本文档用以展示命令，不建议大家使用。
...
\begin{itemize}
    \item \texttt{$\backslash$chapter\{\}}
    \item \texttt{$\backslash$section\{\}}
    \item \texttt{$\backslash$subsection\{\}}
    \item \texttt{$\backslash$caption\{\}}
\end{itemize}
即可。博士使用
\begin{itemize}
    \item \texttt{$\backslash$chapter\{\}}、\texttt{$\backslash$chapteren\{\}}
    \item \texttt{$\backslash$section\{\}}、\texttt{$\backslash$sectionen\{\}}
    \item \texttt{$\backslash$subsection\{\}}、\texttt{$\backslash$subsectionen\{\}}
    \item \texttt{$\backslash$bicaption\{\}\{\}}
\end{itemize}
以显示双语效果。此外，如果需要展示目录和索引，请直接使用\ref{tab:listof}中的命令来打开。
\begin{table}[!htp]
    \zihao{5}
    \bicaption{目录索引}
    {Tableofcontents and  listofX}
    \label{tab:listof}
    \centering
    \begin{tabular}{ll}
        \hline
        索引    & 命令 \\
        \hline
        中文目录    &  \texttt{$\backslash$tableofcontents} \\
        英文目录    &  \texttt{$\backslash$tableofcontentsen} \\
        中文插图索引    &  \texttt{$\backslash$listoffigures} \\
        英文插图索引    &  \texttt{$\backslash$listoffiguresen} \\
        中文表格索引   &  \texttt{$\backslash$listoftables} \\
        英文表格索引   &  \texttt{$\backslash$listoftablesen} \\
        \hline
    \end{tabular}
\end{table}
...
\sectionen{\TeX{} Live suite and other software}
